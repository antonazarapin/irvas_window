...
var changeModalState = function changeModalState(state) {
  var windowForm = document.querySelectorAll('.balcon_icons_img'),
      ...
      windowProfile = document.querySelectorAll('.checkbox'); //Если у нас функция рассчитана на массив - мы можем получить даже один элемент, как All. Отработает 1 раз
...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      ...
      item.appendChild(statusMessage); //Поместить это сообщение нашему айтему
...
var images = function images() {
  ...
  workSection.addEventListener('click', function (e) {
    ...
    if (target && target.classList.contains('preview')) {
      ...
      document.body.style.overflow = "hidden"; //отвечает за то, чтобы подложка не прокручивалась
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener('click', function (e) {
    ...
    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      //для того, чтобы удалить точку и потом чтобы включало всех дочерних
...
var timer = function timer(id, deadline) {
  ...
  var setClock = function setClock(selector, endtime) {
    ...
    updateClock(); //для того, чтобы числа не прыгали пока не запустится интервал, а сразу показывало то, что нужно на странице
